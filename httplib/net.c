	...
	#include <winsock2.h>
#else
	#warning "net.h: platform not supported yet"
#endif
...
extern int listen_net(char *address) {
	...
	int listener = socket(AF_INET, SOCK_STREAM, 0);	// use SOCK_DGRAM for UDP
	if (listener < 0)
		return -SOCKET_ERR;
...
extern int connect_net(char *address) {
	...
	int conn = socket(AF_INET, SOCK_STREAM, 0);	// use SOCK_DGRAM for UDP
	if (conn < 0)
		return -SOCKET_ERR;
